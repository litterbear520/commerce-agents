...
> 到 Step 17 开始写商户 agent 时，才把它们迁移到 `commerce_common`。
...
```
API 调用 → 工具 → 购物车 → 整理任务集 → 幻觉驱动门控（+测试）→ 注入驱动围栏（+测试）
→ 拆包 → 测试基础设施 + 基础 CI
...
任务集贯穿全程：Step 02.5 整理它（跑通搜索和购物车之后），每个 Stage 结束跑一次，Stage H 把它变成定时回归和发布门槛。
